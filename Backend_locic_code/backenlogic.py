...
def create_collage_if_needed(input_path, temp_output_path="temp_collage.png"):
    ...
    return temp_output_path, True  # temporär generiertes Bild
...
# === Hauptfunktion mit kompletter Steuerung ===
def handle_input_file(input_path, loesche_original=False):
    bildpfad = None
    temp_created = False

    try:
        bildpfad, temp_created = create_collage_if_needed(input_path)
        benutze_bild(bildpfad)
...
if __name__ == "__main__":
    testdatei = "./Teaching_files/Signdict_Vidoscraped/Alles.mp4"  # oder .gif, .jpg, .png
    handle_input_file(testdatei, loesche_original=False)  # <- Original behalten
